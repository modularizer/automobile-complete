...
class CoreTrie:
        ...
        for i, ch in enumerate(v):
            ...
            # Handle normal characters
                ...
                if node.completion and node.completion[0] == k:
                    x = node.children.get(k)
                    nn = node.child(ch, node.completion[1:], children=x.children if x is not None else None)
                else:
    ...
    def disable(self, pre: str, post: str) -> None:
        """
        Disable completions for a word by following the prefix and completion path.

        This mirrors Node.disable from preprocess/core.py. It navigates to the prefix
        node and disables completions along the entire completion path.

        Args:
            pre: The prefix string
            post: The completion string to disable
        """
        # Navigate to the prefix node using walk_to
        node = self.walk_to(pre)
        if node is None:
            return
